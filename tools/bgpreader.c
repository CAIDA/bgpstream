...
case 'R':
      ...
      if (!host || !port) {
        print_err_message(RTR_MAX_CNT, "minimum", "arguments (-R)");
      }

#ifdef WITH_SSH
      ssh_user = strsep(&optarg, ",");
      ssh_hostkey = strsep(&optarg, ",");
      ssh_privatekey = strsep(&optarg, ",");
      ...
      bgpstream_set_rtr_config(bs, host, port, ssh_user, ssh_hostkey,
                               ssh_privatekey, 1);
#endif
      break;
